...
print("Food choices: \(foodChoices)")
...
if foodChoices.contains(userFoodChoice) {
    // Prompt for quantity.
    print("Enter the quantity (1-\(maxQuantity)): ", terminator: "")
        ...
        if let quantity = Int(quantityAsString) {
            ...
            if quantity >= 1 && quantity <= maxQuantity {
                ...
                if userFoodChoice == "SUB" {
                    baseReheatTime = subReheatTime
                } else if userFoodChoice == "PIZZA" {
                    baseReheatTime = pizzaReheatTime
                } else if userFoodChoice == "SOUP" {
                    baseReheatTime = soupReheatTime
                }
                // Apply the quantity formula on the base reheat time.
                // To get the actual reheat time. [in seconds]
                let actualReheatTime = Double(baseReheatTime) * Double(quantity + 1) * 0.5
                // Convert pure seconds to a combination of minutes and seconds.
                let minutes = Int(actualReheatTime / Double(secondsPerMinute))
                let seconds = Int(actualReheatTime) % secondsPerMinute
                ...
                print(", IT MUST BE BETWEEN 1 AND \(maxQuantity)")
            }
        } else {
